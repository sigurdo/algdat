...
def sheet_cutting(w, h, p):
    # Skriv koden din her
    # print("w h p:", w, h, p)
    # p = { p_keys[i]: p_values[i] for i in range(len(p_keys)) }
    price0 = p[(w, h,)]
    price1 = 0
    price2 = 0
    price3 = 0
    if w > 1:
        price1 = sheet_cutting(w - 1, h, p) + sheet_cutting(1, h, p)
    if h > 1:
        price2 = sheet_cutting(w, h - 1, p) + sheet_cutting(w, 1, p)
    if w > 1 and h > 1:
        price3 = sheet_cutting(w - 1, h - 1, p) + sheet_cutting(1, 1, p)

    return max(price0, price1, price2, price3)

# def sheet_cutting(w, h, p):
#     # print(type(sheet_cutting), type(sheet_cutting_recusive))
#     # print(list(p.keys()))
#     return sheet_cutting_recusive(w, h, list(p.keys()), list(p.values()))
